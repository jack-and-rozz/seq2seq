...
public:
  ...
  void Compute(OpKernelContext* context) override {
    ...
    // Register in mention_cluster_ids if the span exactly matches a gold span.
    ...
    for (int i = 0; i < num_mentions; ++i) {
      ...
      // iの後ろのmax_ant個 (i-max_antecedents <= j < i) について先行詞かどうかを考える
      for (int j = std::max(0, i - max_antecedents); j < i; ++j) {

	// goldからクラスタidが特定出来て，かつそれが同じ先行詞がある時
        if (mention_cluster_ids[i] >= 0 && mention_cluster_ids[i] == mention_cluster_ids[j]) {
	  // antecedent_count個目のスパンは先行詞である ※jはスパン全体のidx, antecedent_countはiの前のmax_ant個のスパンの中のidx
          labels(i, antecedent_count + 1) = true; 
          null_label = false; // 先行詞を少なくとも1つは持つ
        } else {
