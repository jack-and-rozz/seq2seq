...
class ExperimentManager(ManagerBase):
  ...
  def train(self):
    model = self.create_model(self.config, 'train')
    ...
    self.c_test(use_test_data=False)
    self.g_test(use_test_data=False)
    self.c_test(use_test_data=True)
    self.g_test(use_test_data=True)
  ...
  def train_simultaneously(self, model):
    ...
    for epoch in range(m.epoch.eval(), self.config.max_epoch):
      ...
      if self.use_graph:
        save_as_best = self.g_test_as_valid(m)

      if self.use_coref:
        save_as_best = self.c_test_as_valid(m)
  ...
  def train_one_by_one(self, model):
    ...
    m = self.create_model(self.config, 'train', checkpoint_path=best_ckpt)
    task = 'coref'
    for epoch in range(m.epoch.eval(), self.config.max_epoch):
      _run_task(m, task)
      save_as_best = self.c_test_as_valid(m)
